...
char cur_password[MAXBUF];
char cur_workgroup[MAXBUF];
char cur_username[MAXBUF];
char cur_wins[MAXBUF];
char hosts[MAXHOSTS][MAXBUF];
int maxhost=0;
...
static BOOL query_one(char *lookup, unsigned int lookup_type)
{
	...
	for (j=0;j<count;j++) {
		if (translate_addresses) {
			...
			if (host) {
                                pstrcpy(hosts[maxhost],host -> h_name);
                                maxhost++;
			}
		}
                if (debug){
...
int dosmblogin(char *host)
{
   ...
   if ((dh1 = smbc_opendir(buff))<1) {
      if (debug){
         fprintf(stderr, "DEBUG:  smb login error: smb://: %s\n",
                 strerror(errno));
      }
      return(errno);
   }
...
int main(int argc, char *argv[])
{
   ...
   pstrcpy(cur_password,"");
   pstrcpy(cur_workgroup,"WORKGROUP");
   pstrcpy(cur_username,"nobody");
   pstrcpy(cur_wins,"");
   debug=0;
   while(1){
      c=getopt(argc,argv,"dhu:p:D:B:U:");
      if (c==-1) break;
      switch(c){
         case 'd':debug=255;
                  break;
         ...
         case 'h':help();
                  exit(1);
                  break;
         case 'u':pstrcpy(cur_username,optarg);
                  break;
         case 'D':pstrcpy(cur_workgroup,optarg);
                  break;
         case 'p':pstrcpy(cur_password,optarg);
                  break;
         case 'B':pstrcpy(cur_wins,optarg);
                  got_bcast = True;
                  use_bcast = True;
                  recursion_desired = True;
                  translate_addresses =True;
                  break;
         case 'U':pstrcpy(cur_wins,optarg);
                  got_bcast = True;
                  use_bcast = False;
                  recursion_desired = True;
                  translate_addresses =True;
